...
import { Options, TrackType } from '@/api/api';
...
interface OptionsPanelProps {
  ...
  removeTracks: (type: TrackType[], name?: string) => void;
}
...
export const OptionsPanel: React.FC<OptionsPanelProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      !uniqueAudioNames.includes(options.default_audio_track_name)
    ) {
      setOptions((prev) => ({
        ...prev,
    ...
    // Проверяем субтитры
    if (
      options.default_subtitle_track &&
      !uniqueSubtitleNames.includes(options.default_subtitle_track)
    ) {
      setOptions((prev) => ({
        ...prev,
      ...
      <Accordion.Item eventKey="0" className="border-0">
        ...
        <Accordion.Body>
          <div className="d-flex flex-column gap-3">
                ...
                <div className="d-flex flex-wrap gap-2">
                  {uniqueAudioNames.map((name) => (
                    <Button
                      ...
                      size="sm"
                      onClick={() => removeTracks([TrackType.TRACK_TYPE_AUDIO], name)}
                ...
                <div className="d-flex flex-wrap gap-2">
                  {uniqueSubtitleNames.map((name) => (
                    <Button
                      ...
                      variant="outline-danger"
                      size="sm"
                      onClick={() => removeTracks([TrackType.TRACK_TYPE_SUBTITLE], name)}
                      className="d-inline-flex align-items-center gap-2"
                    >
                      <Trash2 size={18} />
                      {name}
            ...
            {(uniqueAudioNames.length > 0 || uniqueSubtitleNames.length > 0) && (
